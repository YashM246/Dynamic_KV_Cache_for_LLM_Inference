...
class Dynamic_KV_Cache():

    def __init__(self, num_layers, batch_size, num_heads, head_dim, dtype=torch.float32):
        ...
        self.head_dim = head_dim            # Dimensions of each Attention Head
    ...
    def update(self, layer_idx, new_key, new_value):
        ...
        if current_keys is None or current_values is None:
            ...
            # new_key and new_value are 4d tensors squeezed to 3d
            # attention.py sends k_heads (Originally (batch, num_heads, 1, d_k) squeezed to (batch, num_heads, d_k))

            # Need to unsqueeze before adding to cache
            self.cache[layer_idx]["key"] = torch.unsqueeze(new_key, dim=2)
            self.cache[layer_idx]["value"] = torch.unsqueeze(new_value, dim=2)

            return self.cache[layer_idx]["key"], self.cache[layer_idx]["value"]
        
        # Cache tensors are shape (batch, num_heads, seq_len, head_dim)
        # We are growing the sequence length, so dim=2

        self.cache[layer_idx]["key"] = torch.cat((current_keys, torch.unsqueeze(new_key, dim=2)), dim=2)
        self.cache[layer_idx]["value"] = torch.cat((current_values, torch.unsqueeze(new_value, dim=2)), dim=2)
...
@torch.no_grad()
def generate_with_dynamic_cache(model, num_layers, input_seq, num_steps):
    ...
    head_dim = model.d_k
    cache = Dynamic_KV_Cache(num_layers, batch_size, num_heads, head_dim)
...
if __name__ == "__main__":
    ...
    kv_cache = Dynamic_KV_Cache(NUM_LAYERS, BATCH_SIZE, NUM_HEADS, HEAD_DIM)

    print(f"KV Cache Key after Initialization: {kv_cache.cache[0]["key"]}")
    print(f"KV Cache Val after Initialization: {kv_cache.cache[0]["value"]}")
    ...
    _, prefill_time, decode_times = generate_with_dynamic_cache(model=model, num_layers=NUM_LAYERS, input_seq=input_seq, num_steps=STEPS)
    ...
    output_cached, _, _ = generate_with_dynamic_cache(model=model, num_layers=NUM_LAYERS, input_seq=input_seq.clone(), num_steps=STEPS)
